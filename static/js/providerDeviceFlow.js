...
// GitHub Copilot and ChatGPT Subscription device-flow entries were removed:
// company policy permits only Anthropic (Claude) and Google Gemini cloud
// providers (see src.model_context.is_allowed_cloud_provider), and the
// backing /api/copilot/* and /api/chatgpt-subscription/* routes are no
// longer registered in app.py. All consumers below already handle a missing
// key gracefully (`PROVIDER_DEVICE_FLOWS[key] || null`), so leaving this
// empty disables the device-auth flow everywhere without further changes.
