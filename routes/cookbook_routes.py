...
_HF_TOKEN_STATUS_SNIPPET = (
    'if [ -n "$HF_TOKEN" ]; then '
    'echo "[prometheus] HF token: applied"; '
    'else '
    'echo "[prometheus] HF token: NOT SET — gated/private models will be denied. '
    'Add one in Prometheus Cookbook -> Settings -> HuggingFace Token."; '
    'fi'
)
        ...
        patterns = [
            ...
            (
                ...
                [
                    ...
                    {"label": "Prometheus now rewrites MLX repo-id launches to a cached snapshot when one exists", "op": "manual"},
                ],
            ),
