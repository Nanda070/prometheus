...
def setup_session_routes(session_manager: SessionManager, config: dict, webhook_manager=None):
    ...
    def create_session_openai(
        ...
        # Legacy shortcut route that bypasses the ModelEndpoint allowlist in
        # routes/model_routes.py entirely (hardcodes api.openai.com and reads
        # the server's own OPENAI_API_KEY). Company policy permits only
        # Anthropic (Claude) and Google Gemini cloud providers, so this route
        # is disabled outright rather than relying on OPENAI_API_KEY being unset.
        raise HTTPException(403, "OpenAI sessions are disabled by company policy.")
