...
async def do_manage_endpoints(content: str, owner: Optional[str] = None) -> Dict:
    ...
    try:
        if action == "list":
            ...
            from src.model_context import is_allowed_cloud_provider, ALLOWED_CLOUD_PROVIDER_MESSAGE
            if not is_allowed_cloud_provider(base_url):
                return {"error": ALLOWED_CLOUD_PROVIDER_MESSAGE, "exit_code": 1}
