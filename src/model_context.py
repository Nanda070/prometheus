...
def _host_matches(url: str, *domains: str) -> bool:
    """Local, dependency-free mirror of src.llm_core._host_match (importing
    that module here would create a circular import: llm_core -> model_context).
    """
    try:
...
def is_allowed_cloud_provider(base_url: str) -> bool:
    """Company policy allowlist: only Anthropic (Claude) and Google Gemini may
    be added as cloud API providers. Local/self-hosted serving (Ollama, LM
    Studio, llama.cpp, vLLM, SGLang, Cookbook, Tailscale, private IPs) is
    unrestricted. Shared by routes/model_routes.py and
    src/agent_tools/admin_tools.py so both the HTTP API and the agent's
    manage_endpoints tool enforce the same rule.
    """
...
ALLOWED_CLOUD_PROVIDER_MESSAGE = (
    "Only Anthropic (Claude) and Google Gemini cloud providers are permitted "
    "by company policy. Local/self-hosted endpoints are unaffected."
)
