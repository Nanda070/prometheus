...
# GitHub Copilot and ChatGPT Subscription device-flow login are disabled by
# company policy: only Anthropic (Claude) and Google Gemini cloud providers
# are permitted (see src.model_context.is_allowed_cloud_provider). Not
# registering these routers 404s /api/copilot/* and
# /api/chatgpt-subscription/* entirely, closing the OAuth-provisioning path
# that otherwise bypasses the ModelEndpoint allowlist checks in
